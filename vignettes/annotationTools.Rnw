...
In 2014 NCBI introduced a new, dense source of orthologs for vertebrate species (see \url{https://www.ncbi.nlm.nih.gov/news/03-13-2014-gene-provides-orthologs-regions/}). It is for instance used to provide links to orthologs in NCBI's Gene portal (\url{https://www.ncbi.nlm.nih.gov/gene}). 

The complete flat file version of this database can be retrieved from \url{ftp.ncbi.nlm.nih.gov/gene/DATA/gene_group.gz}. A partial version is provided with this package

<<>>=
gene_groupFile<-system.file('extdata','gene_group_part.data',package='annotationTools')
group_gene<-read.delim(gene_groupFile,header=TRUE)
@

We can easily mine 'gene\_group' by setting the \texttt{tableType} argument in \Rfunction{getHOMOLOG} to \texttt{'gene_group'} (instead of the \texttt{'homologene'} default)
