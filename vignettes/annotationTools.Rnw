...
We can easily mine 'gene\_group' by setting the \texttt{tableType} argument in \Rfunction{getHOMOLOG} to \texttt{'gene\_group'} (instead of the \texttt{'homologene'} default)
